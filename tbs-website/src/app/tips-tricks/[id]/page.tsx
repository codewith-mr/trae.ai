...
export default function TipPage() {
  ...
  // Compute recommended tips: prefer same category, then fallback to others
  const sameCategory = tipsAndTricks.filter(t => t.id !== tip.id && t.category === tip.category);
  const otherTips = tipsAndTricks.filter(t => t.id !== tip.id && t.category !== tip.category);
  const recommendedTips = [...sameCategory, ...otherTips].slice(0, 3);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {recommendedTips.length > 0 && (
        <div className="mt-12">
          <h2 className="text-2xl font-bold text-gray-800">Recommended Tips & Tricks</h2>
